...
public class FireResourceReloadListener implements SimpleResourceReloadListener<HashMap<Identifier, Fire>> {
  ...
  public CompletableFuture<HashMap<Identifier, Fire>> load(ResourceManager manager, Profiler profiler, Executor executor) {
    return CompletableFuture.supplyAsync(() -> {
      ...
      for (Identifier resourceId : manager.findResources("fires", path -> path.endsWith(".json"))) {
        String jsonIdentifier = resourceId.getPath();
        try {
          JsonObject fire = JsonParser.parseReader(new Gson().newJsonReader(new InputStreamReader(manager.getResource(resourceId).getInputStream()))).getAsJsonObject();
          try {
