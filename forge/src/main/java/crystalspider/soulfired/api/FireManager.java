...
public abstract class FireManager {
  ...
  /**
   * Returns the material 0 of the {@link Fire} registered with the given {@code id}.
   * <p>
   * Returns {@code null} if no {@link Fire} was registered with the given {@code id}.
   * 
   * @param id
   * @return the material 0 of the {@link Fire} registered with the given {@code id}.
   */
  public static final Material getMaterial0(String id) {
    if (isFireId(id)) {
      return fires.get(id).getMaterial0();
    }
    return null;
  }

  /**
   * Returns the material 1 of the {@link Fire} registered with the given {@code id}.
   * <p>
   * Returns {@code null} if no {@link Fire} was registered with the given {@code id}.
   * 
   * @param id
   * @return the material 1 of the {@link Fire} registered with the given {@code id}.
   */
  public static final Material getMaterial1(String id) {
    if (isFireId(id)) {
      return fires.get(id).getMaterial1();
    }
    return null;
  }
   ...
   * @param damageSource
   * @param damage
   * @param invertedHealAndHarm
   * @return whether the {@code entity} has been harmed.
   */
  private static final boolean harmOrHeal(Entity entity, DamageSource damageSource, float damage, boolean invertedHealAndHarm) {
    if (damage > 0) {
      if (entity instanceof LivingEntity) {
        LivingEntity livingEntity = (LivingEntity) entity;
        if (livingEntity.isInvertedHealAndHarm() && invertedHealAndHarm) {
